...
character_width = character_size[0] #캐릭터 가로 크기, 위에 함수에 첫번째 값이 가로래 시발
character_height = character_size[1] #캐릭터 세로 크기 위에 함수에 두번재 값이 세로래 시발
...
#폰트 설정
game_font = pygame.font.Font(None,40) #폰트 객체 생성 (폰트,크기) None은 기본값으로 설정됨

#총 시간
total_time = 100

#시작 시간
start_ticks = pygame.time.get_ticks() #현재 이 순간 tick을 받아와 시작 시간으로 설정
...
while running :
    ...
    #타이머 집어넣기
    #경과 시간 
    elapsed_time = (pygame.time.get_ticks() - start_ticks)/1000  #경과 시간(ms)을 초로 보여주기, 계속 갱신되는 현재 시간에서 맨위 시작시간을 뺌으로서 경과 시간 나타내기
    
    timer = game_font.render(str(int(total_time - elapsed_time)), True, (0,0,0)) #출력할 글자,true,색깔 (int로 바꾸는 작업은 소수점 생략, str은 글자로 출력해야하니까)
